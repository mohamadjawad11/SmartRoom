...
const Meetings = () => {
  ...
  const isUpcomingOrOngoing = (meeting) => {
  ...
  const fetchMeetings = async () => {
    try {
      ...
      setInvitedMeetings(data.filter(isUpcomingOrOngoing)); // ⬅️ only future or ongoing

      const response2 = await fetch("http://localhost:5091/api/Booking/my-created-meetings", {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data2 = await response2.json();
      setCreatedMeetings(data2.filter(isUpcomingOrOngoing)); // ⬅️ only future or ongoing
  ...
  const handleJoinMeeting = async (bookingId) => {
    try {
      ...
      navigate(`/meeting-inside/${bookingId}`);
    } catch (error) {
      alert(error?.response?.data?.message || "Failed to join the meeting.");
      ...
      <div className="meetings-list">
        ...
        {invitedMeetings.map((meeting) => (
          <div className="meeting-card" key={meeting.id}>
            ...
            <button onClick={() => handleJoinMeeting(meeting.bookingId)}>Join Meeting</button>
          </div>
        ))}
      </div>
    </div>
